  ...
  return (
    <div style={{ maxWidth: 900 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'2rem', flexWrap:'wrap', gap:'1rem' }}>
        <div>
          <h1 style={{ fontFamily:'Playfair Display,serif', fontSize:'clamp(1.4rem,3vw,2rem)', fontWeight:700, color:'white', marginBottom:'0.3rem' }}>⭐ Testimonials</h1>
          <p style={{ color:'rgba(255,255,255,0.35)', fontSize:'0.82rem' }}>Manage client testimonials</p>
        </div>
      ...
      {showForm && (
        <div style={{ background:'#1a1a1a', border:'1px solid rgba(212,168,83,0.2)', borderRadius:8, padding:'1.75rem', marginBottom:'1.5rem' }}>
          <h3 style={{ color:'white', fontSize:'1rem', marginBottom:'1.25rem', fontFamily:'Playfair Display,serif' }}>Add New Testimonials</h3>
          ...
          <button style={{ background:'linear-gradient(135deg,#D4A853,#B8935A)', color:'white', border:'none', padding:'0.65rem 1.5rem', borderRadius:4, cursor:'pointer', fontSize:'0.74rem', fontWeight:600, letterSpacing:'0.1em' }}>Save Testimonials</button>
        </div>
      )}
        ...
        {[1,2,3,4,5].map(i=>(
          <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1rem 1.25rem', borderBottom:'1px solid rgba(255,255,255,0.05)', flexWrap:'wrap', gap:'0.75rem' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'0.875rem' }}>
              <div style={{ width:42, height:42, borderRadius:4, background:'rgba(212,168,83,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1rem', flexShrink:0 }}>⭐</div>
              <div>
                <p style={{ color:'rgba(255,255,255,0.85)', fontSize:'0.85rem', fontWeight:500, marginBottom:2 }}>Sample Testimonials {i}</p>
